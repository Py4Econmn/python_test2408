...
dfw['educ_s'] = dfw['educ_s'].astype(str)
# dfw['educ_s'].fillna('ганц бие', inplace=True)
dfw['educ_s'].replace('nan', 'ганц бие', inplace=True)
...
df_plot = dfw # dfw[dfw['age_female'] - dfw['age_male'] > 10]  #dfw #[dfw['age_h']<30] #dfw[dfw['age_male']-dfw['age_female']>=10]
...
plt.xlabel('Эрэгтэй')
plt.ylabel('Эмэгтэй')
plt.title('Өрхийн гишүүдийн боловсролын байдал (2022 он)')
...
# all_education_levels = set(dfw['educ_male'].unique()) | set(dfw['educ_female'].unique())
